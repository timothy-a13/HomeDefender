...
from Risk import create_rister, RiskClassifier
...
def run(
    ...
    rister_list:list[RiskClassifier] = []
    for i in range(bs):
        tracker = create_tracker(tracking_method, tracking_config, reid_weights, device, half)
        tracker_list.append(tracker, )
        rister = create_rister(tracking_config)
        rister_list.append(rister, )
    ...
    for frame_idx, batch in enumerate(dataset):
        ...
        for i, det in enumerate(preds):  # detections per image
            ...
            if det is not None:
                ...
                states[i] = rister_list[i].update(outputs[i], fps=30)
