    ...
    def update(self, det, weapon_det, fps=24):
        ...
        if det is None:
            ...
            if self.t_age >= BGN_FRAME and self.t_age - self.lastp >= fps // 3:
                ...
                thredhold = (self.size + 175) / 230
                self.th.append(thredhold)

                # 速度計算區
                diff = math.hypot(abs(cur_x - self.pre_x), abs(cur_y - self.pre_y))
                self.speeds.append(diff / (self.t_age - self.lastp))
                if len(self.speeds) > 2: self.speeds.pop(0)
                speeds = np.array(self.speeds)
                self.ys.append(self.speeds[-1])
                ...
                if self.pre_angle is not None:
                    ...
                    if len(self.t_angle_list) >= 3:
                        t_angle = self.t_angle_list[-3]
                ...
                if np.all(speeds < thredhold) and self.state == 1:
                    #print(f'Is wait: {self.state == 1}')
                    self.min = self.max = None
                    self.total_angle = 0

                if self.min is not None and self.max is not None:
                    if np.all(speeds < thredhold) and self.max - self.min > 120 and self.state < 1:
                        self.state = State.WAIT
                        state_change = np.concatenate((det[0:6], [int(State.WAIT), self.weapon_state]))
                    elif not np.all(speeds < thredhold) and self.max - self.min > 150 and self.state < 2:
