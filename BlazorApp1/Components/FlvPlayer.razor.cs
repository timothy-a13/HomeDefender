...
{
    public partial class FlvPlayer : IAsyncDisposable
    {
        [Inject] IJSRuntime? JS { get; set; }
        private DotNetObjectReference<FlvPlayer>? _instance { get; set; }
        ...
        /// 資源類型
        [Parameter]
        public string? SrcType { get; set; } = "video/x-flv";
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            try
            {
                if (firstRender)
                {
                    ...
                    try
                    {
                        await JS!.InvokeVoidAsync("load_flv", _instance, "video_" + Id, Option);
                    }
                    catch (Exception e)
                    {
                        info = e.Message;
